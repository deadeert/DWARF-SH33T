...
class sample_processor_t(idaapi.processor_t):
	...
	"DS"
		...
		reg_first_sreg = 33 # index of CS
		reg_last_sreg  = 34 # index of DS
...
# this list will be extremely limited.
		...
		{'name': 'SKIP', 'feature': CF_USE1, 'cmt' : 'jmp imm', 'stack_inc' :0}, 
...
#							print('[!]Error while creating switch for branch instruction: %s'%e.__str__()) 
					else: 
							add_cref(insn.ea, insn.ea + insn.size, fl_F)
		...
		def notify_ana(self, insn):
				...
				if b == 0x03:              # ADD
					...
					insn.itype = 1 
				elif b == 0x8: 						 # LIT Byte Unsigned  
					insn.itype = 2 
					insn.Op1.type = o_imm
					insn.Op1.dtype = dt_byte
					insn.Op1.value = insn.get_next_byte()
				elif b == 0x9:  					 # LIT Byte Signed
					insn.itype = 3 
					insn.Op1.type = o_imm
					insn.Op1.dtype = dt_byte
					insn.Op1.value = insn.get_next_byte()
				elif b == 0xa: 						# LIT  Word UnSigned
					insn.itype = 4 
					insn.Op1.type = o_imm
					insn.Op1.dtype = dt_word
					insn.Op1.value = insn.get_next_word()
				elif b == 0xb:						# LIT Word Signed 
					insn.itype = 5 
					insn.Op1.type = o_imm
					insn.Op1.dtype = dt_word
					insn.Op1.value = insn.get_next_word()
				elif b == 0xc: 					 # LIT Double Unsigned
					insn.itype = 6 
					insn.Op1.type = o_imm
					insn.Op1.dtype = dt_dword
					insn.Op1.value = insn.get_next_dword()
				elif b == 0xd:  				 # LIT Double Signed
					insn.itype = 7 
					insn.Op1.type = o_imm
					insn.Op1.dtype = dt_dword
					insn.Op1.value = insn.get_next_dword()
				elif b == 0xe: 					# LIT Qword Unsigned 
					insn.itype = 8 
					insn.Op1.type = o_imm
					insn.Op1.dtype = dt_qword
					insn.Op1.value = insn.get_next_qword()
				elif b == 0xf: 				 # LIT Qword Signed 
					insn.itype = 9 
					insn.Op1.type = o_imm
					insn.Op1.dtype = dt_qword
					insn.Op1.value = insn.get_next_qword()
				elif b == 0x10: 			# LIT ULEB128
					insn.itype = 10 
					insn.Op1.type = o_imm
					insn.Op1.dtype = dt_qword
					...
					insn.itype = 14 
				elif b == 0x15:			#PICK [0..255] Note DWARF2 only handle 64 stack entries
					insn.itype = 15 
					insn.Op1.type = o_imm
					insn.Op1.dtype = dt_byte
					insn.Op1.value = insn.get_next_byte() #TODO :
				elif b == 0x16:			# SWAP 
					insn.itype = 16 
				elif b == 0x17:			# ROT
					insn.itype = 17 
				elif b == 0x18: 		# XDEREF wtf..
					insn.itype = 18 	
				elif b == 0x19:			# ABS
					insn.itype = 19 
				elif b == 0x1A:			# AND
				...
				elif b == 0x28: #branch (pop value if not jump to ea+ word signed value). 
					insn.itype = 35 
					insn.Op1.type = o_near
					insn.Op1.dtype = dt_word
					nea = insn.get_next_word()
					...
					insn.itype =41 
				elif b in range(0x30,0x50): #litn 
					insn.itype =42 
					insn.Op1.type = o_reg
					insn.Op1.reg = b-0x30 
				elif b in range(0x50,0x70): #regn 
					insn.itype =43
					insn.Op1.type = o_reg
					insn.Op1.reg = b-0x50-1
				elif b in range (0x70,0x90) : #breg 
					insn.type=44
					insn.Op1.type = o_reg
